...
import { mockCertidoes } from "@/lib/mock-data";
...
export default function CerticoesPage() {
  const validas = mockCertidoes.filter((c) => c.status === "valida");
  const vencendo = mockCertidoes.filter((c) => c.status === "vencendo");
  const vencidas = mockCertidoes.filter((c) => c.status === "vencida");
  ...
  return (
    <div className="p-8">
        ...
        >
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: "#991B1B" }}>
              Atenção: certidões com situação irregular
            </p>
      ...
      >
        <h3 className="text-sm font-semibold mb-3" style={{ color: "#334155" }}>
          Prazos de Validade Padrão
        </h3>
        <div className="grid grid-cols-3 gap-4 text-xs" style={{ color: "#64748B" }}>
          <div>
            <strong style={{ color: "#334155" }}>Federal (Receita + PGFN)</strong>
            <br />180 dias
          </div>
          <div>
            <strong style={{ color: "#334155" }}>FGTS (Caixa)</strong>
            <br />30 dias
          </div>
          <div>
            <strong style={{ color: "#334155" }}>Trabalhista (TST)</strong>
            <br />180 dias
          </div>
          <div>
            <strong style={{ color: "#334155" }}>Estadual (SEFAZ-AL)</strong>
            <br />Variável
          </div>
          <div>
            <strong style={{ color: "#334155" }}>Municipal (SEMFAZ Maceió)</strong>
            <br />Variável
          </div>
          <div>
            <strong style={{ color: "#334155" }}>Falência/Concordata (TJAL)</strong>
            <br />Variável
          </div>
        </div>
      </div>
