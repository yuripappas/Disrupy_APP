      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0 mr-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: "#ECFDF5", color: "#059669" }}>Ativo</span>
          </div>
          <h3 className="font-semibold text-sm leading-tight" style={{ color: "#0F172A" }}>{f.razao_social}</h3>
          <p className="text-xs mt-1 font-mono" style={{ color: "#94A3B8" }}>{f.cnpj}</p>
        </div>
      </div>
      <div className="rounded-lg p-3 space-y-2" style={{ backgroundColor: "#F8FAFC" }}>
        {f.contato_nome && <p className="text-xs font-medium" style={{ color: "#334155" }}>{f.contato_nome}</p>}
        <div className="flex flex-col gap-1.5">
          {f.contato_email && (
            <a href={`mailto:${f.contato_email}`} className="flex items-center gap-1.5 text-xs hover:underline" style={{ color: "#2E60FF" }}>
              <Mail className="w-3 h-3" />{f.contato_email}
            </a>
          )}
          {f.contato_whatsapp && (
